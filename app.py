...
if mode=="📥 データ取り込み":
    ...
    sel=st.selectbox("フォルダタイプ",list(folder_options.keys()))
    folder_id=st.text_input("Google Drive フォルダ ID",value=folder_options[sel])
    c1,c2=st.columns(2)
...
if mode=="📊 可視化":
    ...
    try:
        df=pd.read_sql(
            sa.select(tbl).where(tbl.c.機種==machine_sel, tbl.c.date.between(vis_start,vis_end)).order_by(tbl.c.date),
            eng
        )
    except Exception as e:
    ...
    if st.sidebar.checkbox("全台平均を表示", value=False):
        ...
        slots=[int(r[0]) for r in eng.connect().execute(
            sa.select(tbl.c.台番号).where(tbl.c.機種==machine_sel, tbl.c.date.between(vis_start,vis_end)).distinct().order_by(tbl.c.台番号)
        ) if r[0] is not None]
